...
{- Convert the graph from a list of adjacency lists to
   a vertex-vector of neighbours map
-}
...
{- Delete an edge in the graph
   ...
   toIdx -- the index of the to-node in the vector pointed to by the
   from-node
-}
...
{- Test if the player who makes the next move has a winning strategy if going
   to any of the next states (vertice) with index >= nextIdx. The nextIdx
   parameter is used for recursion.

   nextIdx -- the index of the next vertex to test in the vector pointed to by
   the current vertex
-}
